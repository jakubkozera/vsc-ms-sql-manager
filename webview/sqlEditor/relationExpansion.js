...
/**
 * Get expansion state for a row
 * @returns {Object} { hasExpanded: boolean, hasCollapsed: boolean }
 */
function getRowExpansionState(tableId, rowIndex) {
    let hasExpanded = false;
    let hasCollapsed = false;
    
    const prefix = `${tableId}-${rowIndex}-`;
    
    for (const [key, value] of expandedRows.entries()) {
    ...
    return { hasExpanded, hasCollapsed };
}

/**
 * Collapse all expanded relations for a row
 */
function collapseRowRelations(tableId, rowIndex) {
    const prefix = `${tableId}-${rowIndex}-`;
    
    for (const [key, value] of expandedRows.entries()) {
        if (key.startsWith(prefix) && !value.isCollapsed) {
            const row = value.element;
            const height = parseInt(row.style.height || '0');
            ...
            // Shift rows below up
            if (row.parentNode) {
                shiftRowsBelow(row.parentNode, rowIndex, -height);
            }
            
            // Update chevron
            const columnName = key.split('-').pop();
            if (row.parentNode) {
                const sourceRow = Array.from(row.parentNode.children).find(tr => 
                    tr.dataset.rowIndex === String(rowIndex) && !tr.classList.contains('expanded-row-content')
                );
...
/**
 * Expand all collapsed relations for a row
 */
function expandRowRelations(tableId, rowIndex) {
    const prefix = `${tableId}-${rowIndex}-`;
